...
class CapacityIndustryController {
  constructor() {
  ...
  getCapacityIndustry = async (req, res) => {
    try {
      ...
      const result = await request.execute('sp_CapacityIndustry');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching Capacity Industry:', error);
      res.status(500).json({ error: 'An error occurred while fetching Capacity Industry', details: error.message });
    }
  }
...
module.exports = new CapacityIndustryController();
