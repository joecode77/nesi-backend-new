...
class CustomerPopulationByServiceBandsController {
  constructor() {
  ...
  getCustomerPopulationByServiceBands = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_CustomerPopulationByServiceBands');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching Customer Population By Service Bands:', error);
      res.status(500).json({ error: 'An error occurred while fetching Customer Population By Service Bands', details: error.message });
    }
  }
...
module.exports = new CustomerPopulationByServiceBandsController();
