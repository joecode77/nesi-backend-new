...
class DiscoLoadOfftakeController {
  constructor() {
  ...
  getDiscoLoadOfftake = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_DiscoLoadOfftake');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching DISCO Load Offtake:', error);
      res.status(500).json({ error: 'An error occurred while fetching Disco Load Offtake', details: error.message });
    }
  }
...
module.exports = new DiscoLoadOfftakeController();
