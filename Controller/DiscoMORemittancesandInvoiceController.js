...
class DiscoMORemittancesandInvoiceController {
  constructor() {
  ...
  getDiscoRemittancesandInvoice = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_DiscoMORemittances_Invoice');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching DISCO MO Remittances and Invoice:', error);
      res.status(500).json({ error: 'An error occurred while fetching Disco MO Remittances and Invoice', details: error.message });
    }
  }
...
module.exports = new DiscoMORemittancesandInvoiceController();
