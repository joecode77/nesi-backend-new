...
class DiscoMeteringStatusController {
  constructor() {
  ...
  getDiscoMeteringStatus = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_DiscoMeteringStatus');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching DISCO Metering Status:', error);
      res.status(500).json({ error: 'An error occurred while fetching Disco Metering Status', details: error.message });
    }
  }
...
module.exports = new DiscoMeteringStatusController();
