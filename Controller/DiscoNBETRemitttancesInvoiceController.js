...
class DiscoNBETRemitttancesInvoiceController {
  constructor() {
  ...
  getDiscoNBETRemittancesInvoice = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_DiscoNBET_Remittances_Invoice');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching DISCO NBET Remittances and Invoice:', error);
      res.status(500).json({ error: 'An error occurred while fetching Disco NBET Remittances and Invoice', details: error.message });
    }
  }
...
module.exports = new DiscoNBETRemitttancesInvoiceController();
