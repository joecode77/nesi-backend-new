...
class EnergyGeneratedSourceController {
  constructor() {
  ...
  getEnergyGeneratedSource = async (req, res) => {
    try {
      ...
      const result = await request.execute('Sp_EnergyGeneratedSource');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching Energy Generated Source:', error);
      res.status(500).json({ error: 'An error occurred while fetching Energy Generated Source', details: error.message });
    }
  }
...
module.exports = new EnergyGeneratedSourceController();
