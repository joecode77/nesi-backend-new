...
class EnergyInjectedandDeliveredController {
  constructor() {
  ...
  getEnergyInjectedandDelivered = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_EnergyInjectedandDelivered');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching Energy Injected and Delivered:', error);
      res.status(500).json({ error: 'An error occurred while fetching Energy Injected and Delivered', details: error.message });
    }
  }
...
module.exports = new EnergyInjectedandDeliveredController();
