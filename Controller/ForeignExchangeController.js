...
class ForeignExchangeController {
  ...
  getForeignExchange = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_FX');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching Foreign Exchange:', error);
      res.status(500).json({ error: 'An error occurred while fetching Foreign Exchange', details: error.message });
    }
  }
...
module.exports = new ForeignExchangeController();
