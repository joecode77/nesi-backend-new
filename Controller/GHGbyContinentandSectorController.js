...
class GHGbyContinentandSectorController {
  constructor() {
  ...
  getGHGbyContinentandSector = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_GHGbyContinentandSector');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching GHG by Continent and Sector:', error);
      res.status(500).json({ error: 'An error occurred while fetching GHG by Continent and Sector', details: error.message });
    }
  }
...
module.exports = new GHGbyContinentandSectorController();
