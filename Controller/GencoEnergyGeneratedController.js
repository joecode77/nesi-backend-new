...
class GencoEnergyGeneratedController {
  constructor() {
  ...
  getGencoEnergyGenerated = async (req, res) => {
    try {
      ...
      const result = await request.execute('Sp_GencoEnergyGenerated');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching Genco Energy Generation:', error);
      res.status(500).json({ error: 'An error occurred while fetching Genco Energy Generated', details: error.message });
    }
  }
...
module.exports = new GencoEnergyGeneratedController();
