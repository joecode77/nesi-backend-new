...
class GencoInvoicetoNBETController {
  constructor() {
  ...
  getGencoInvoicetoNBET = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_GencoInvoicetoNBET');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching Genco Invoice to NBET:', error);
      res.status(500).json({ error: 'An error occurred while fetching Genco Invoice to NBET', details: error.message });
    }
  }
...
module.exports = new GencoInvoicetoNBETController();
