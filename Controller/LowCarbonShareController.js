...
class LowCarbonShareController {
  constructor() {
  ...
  getLowCarbonShare = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_LowCarbonShare');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching Low Carbon Share:', error);
      res.status(500).json({ error: 'An error occurred while fetching Low Carbon Share', details: error.message });
    }
  }
...
module.exports = new LowCarbonShareController();
