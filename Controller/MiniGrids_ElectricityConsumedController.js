...
class MiniGrids_ElectricityConsumedController {
  constructor() {
  ...
  getMiniGridsElectricityConsumed = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_MiniGridsElectricityConsumedMWh');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching MiniGrids Electricity Consumed:', error);
      res.status(500).json({ error: 'An error occurred while fetching MiniGrids Electricity Cosumed', details: error.message });
    }
  }
...
module.exports = new MiniGrids_ElectricityConsumedController();
