...
class MiniGrids_InstalledCapacityController {
  constructor() {
  ...
  getMiniGridsInstalledCapacity = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_MinigridsInstalledCapacity');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching MiniGrids Installed Capacity:', error);
      res.status(500).json({ error: 'An error occurred while fetching MiniGrids Installed Capacity', details: error.message });
    }
  }
...
module.exports = new MiniGrids_InstalledCapacityController();
