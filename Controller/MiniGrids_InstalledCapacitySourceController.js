...
class MiniGrids_InstalledCapacitySourceController {
  constructor() {
  ...
  getMiniGridsInstalledCapacitySource = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_MiniGridsInstalledCapacitySource');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching MiniGrids Installed Capacity Source:', error);
      res.status(500).json({ error: 'An error occurred while fetching MiniGrids Installed Capacity Source', details: error.message });
    }
  }
...
module.exports = new MiniGrids_InstalledCapacitySourceController();
