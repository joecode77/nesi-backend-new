...
class MiniGrids_NumberofCommunititesbyRegisteredProgramsController {
  constructor() {
  ...
  getMiniGridsNumofCommunitiesbyRegPrograms = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_MinigridsNumofRegisteredPrograms');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching Mini Grids Number of Communities by Registered Programs:', error);
      res.status(500).json({ error: 'An error occurred while Number of Communities by Registered Programs', details: error.message });
    }
  }
...
module.exports = new MiniGrids_NumberofCommunititesbyRegisteredProgramsController();
