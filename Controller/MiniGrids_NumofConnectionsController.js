...
class MiniGrids_NumofConnectionsController {
  constructor() {
  ...
  getMiniGridsNumofConnections = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_MinigridsNumofConnections');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching MiniGrids Number of Connections:', error);
      res.status(500).json({ error: 'An error occurred while fetching MiniGrids Number of Connections', details: error.message });
    }
  }
...
module.exports = new MiniGrids_NumofConnectionsController();
