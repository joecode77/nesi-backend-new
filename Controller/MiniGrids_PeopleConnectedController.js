...
class MiniGrids_PeopleConnectedController {
  constructor() {
  ...
  getMiniGridsPeopleConnected = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_MiniGrids_PeopleConnected');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching MiniGrids People Connected:', error);
      res.status(500).json({ error: 'An error occurred while fetching MiniGrids People Connected', details: error.message });
    }
  }
...
module.exports = new MiniGrids_PeopleConnectedController();
