...
class MiniGrids_TotalInvestmentController {
  constructor() {
  ...
  getMiniGridsTotalInvestment = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_MiniGridsTotalInvestment');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching Mini Grids Total Investment:', error);
      res.status(500).json({ error: 'An error occurred while fetching Mini Grids Total Investment', details: error.message });
    }
  }
...
module.exports = new MiniGrids_TotalInvestmentController();
