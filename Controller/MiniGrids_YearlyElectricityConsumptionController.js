...
class MiniGrids_YearlyElectricityConsumptionController {
  constructor() {
  ...
  getMiniGridsYearlyEnergyConsumption = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_MiniGridsYearlyElectricityConsumption');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching MiniGrids Yearly Electricity Consumption:', error);
      res.status(500).json({ error: 'An error occurred while fetching MiniGrods Yearly Electricity Consumption', details: error.message });
    }
  }
...
module.exports = new MiniGrids_YearlyElectricityConsumptionController();
