...
class NBETPaymenttoGencoController {
  constructor() {
  ...
  getNBETPaymenttoGenco = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_NBETPaymenttoGenco');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching NBET Payment to Genco:', error);
      res.status(500).json({ error: 'An error occurred while fetching NBET Payment to Genco', details: error.message });
    }
  }
...
module.exports = new NBETPaymenttoGencoController();
