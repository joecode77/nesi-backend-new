...
class ShareofGenerationController {
  constructor() {
  ...
  getShareofGeneration = async (req, res) => {
    try {
      ...
      const result = await request.execute('sp_ShareofGeneration');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching Genco Share of Generation:', error);
      res.status(500).json({ error: 'An error occurred while fetching Genco Genco Share of Generation', details: error.message });
    }
  }
...
module.exports = new ShareofGenerationController();
