...
class TransmissionLossFactorController {
  constructor() {
  ...
  getTransmissionLossFactor = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_TransmissionLossFactor');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching Transmission Loss Factor:', error);
      res.status(500).json({ error: 'An error occurred while fetching Transmsission Loss Factor', details: error.message });
    }
  }
...
module.exports = new TransmissionLossFactorController();
