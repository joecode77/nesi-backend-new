...
class WorldEnergyMixController {
  constructor() {
  ...
  getWorldEnergyMix = async (req, res) => {
    try {
      ...
      const result = await request.execute('SP_WorldEnergyMix');

      // Send the result to the client
      res.json(result.recordset);
    } catch (error) {
      console.error('Error fetching World Energy Mix:', error);
      res.status(500).json({ error: 'An error occurred while World Energy Mix', details: error.message });
    }
  }
...
module.exports = new WorldEnergyMixController();
